...
[train-location-and-visitors](https://github.com/acrylic-style/train-location-and-visitors-server)のためのAPIサーバーです。

## 機能

- ([ODPT](https://www.odpt.org) (公共交通オープンデータ)からデータを受け取り、フォーマットし、クライアントに送信します。
- 以下のデータタイプが使用可能です。
    - `odpt:TrainTimetable` (列車の時刻表)
    - `odpt:Train` (列車の遅延を含むリアルタイム位置情報)
    - `odpt:Station` (駅データ)
- 以下のデータタイプはいずれおそらく使用可能にする予定です。
    - GTFS/GTFS-JP
    - GTFS-RT
- [Garoon](https://garoon.cybozu.co.jp/) (スケジュール管理SaaS)から「来客」をキーワードに含む予定を取得します。
...
[staticData.ts](src/staticData.ts)の内容を返します。
